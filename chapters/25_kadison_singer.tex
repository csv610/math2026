...
\subsection{Informal}
In quantum physics, a ``state'' tells you the probabilities for every possible measurement you could make on a system. A ``pure state'' is one that is as definite as the laws of physics allow. Now imagine you only know the probabilities for \emph{some} measurements---say, those along the left-right, up-down, and forward-backward directions. Can you fill in the rest to get a complete pure state, and is there only one way to do it?

This question, posed by Richard Kadison and Isadore Singer in 1959, sounds like pure physics, but it turned out to be equivalent to a surprisingly down-to-earth puzzle: given a collection of vectors in space, can you always split them into a few groups, each of which is nearly as well-behaved as a set of mutually perpendicular axes?
