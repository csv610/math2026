...
\section{The Problem}
In quantum mechanics, a physical system is described by a \emph{state}---a mathematical object that encodes the probabilities of all possible measurements. A \emph{pure state} is one that is as determined as possible: it gives definite answers to some measurements, and random answers to others. Now suppose you have a pure state that only tells you about \emph{some} measurements---say, measurements made along the coordinate axes. Can you extend this partial information to a pure state that covers \emph{all} possible measurements, and is that extension unique?

This question, posed by Richard Kadison and Isadore Singer in 1959, sounds like it belongs in a physics textbook. But it turned out to be equivalent to a surprisingly concrete problem about vectors in high-dimensional space: \emph{can you always chop up a collection of vectors into a few groups, each of which is ``almost orthonormal''---that is, nearly as well-behaved as a set of perpendicular axes?}

In 1959, Richard Kadison and Isadore Singer posed a fundamental question in mathematical physics and operator algebras \cite{kadison1959}: \emph{Does every pure state on the diagonal subalgebra of $B(\ell^2)$ have a unique extension to a pure state on $B(\ell^2)$?}

While formulated in the context of quantum mechanics, Joel Anderson showed in 1979 that the Kadison--Singer problem is equivalent to a simple discretization question about partitioning vectors in Hilbert spaces \cite{anderson1979}:
