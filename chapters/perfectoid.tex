...
For decades, number theorists had a pocket dictionary that could translate between two mathematical languages: the language of equations over ``ordinary'' numbers (characteristic 0) and the language of equations where all numbers eventually wrap around to zero (characteristic $p$). But the dictionary only worked one word at a time---for individual, well-behaved spaces. Scholze's insight was to build a \emph{universal translator}: a new kind of mathematical object that lives in both languages simultaneously. Once you can speak both dialects at once, you discover that they are not merely analogous---they are \emph{the same language} viewed through different lenses. The trick was to find the right ``grammar'' (a new topology) that makes the translation perfect.
