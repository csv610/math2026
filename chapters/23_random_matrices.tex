...
\section{The Problem}
Random Matrix Theory (RMT) studies the eigenvalues of matrices whose entries are random variables. Introduced by John Wishart in statistics in 1928 and popularized by Eugene Wigner in nuclear physics in the 1950s, the field focuses on describing the distributions of eigenvalues for large matrices ($N \to \infty$).

The fundamental global result is Wigner's Semicircle Law, which states that the empirical spectral distribution of a normalized Wigner matrix converges to a semicircle:
\begin{equation}
