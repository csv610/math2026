...
Imagine trying to prove every lock in a city can be opened by keys from a single key-ring. Frey and Ribet showed that if there were even one lock with no matching key, it would produce an ``impossible'' lock---a lock so strange it could not exist. So proving every lock is on the ring suffices. Wiles's revolutionary move was to stop checking locks one by one. Instead, he studied the *entire space of possible locks* at once, and proved that the key-ring ``reaches'' all of them. The core insight is a *reduction*: a 350-year-old puzzle about whole numbers is secretly a question about whether two vast mathematical universes---elliptic curves and modular forms---are the same universe in disguise.
