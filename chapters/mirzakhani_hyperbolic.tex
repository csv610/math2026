% Chapter 19: Maryam Mirzakhani's Work on Hyperbolic Surfaces
...
\textbf{Background.} A hyperbolic surface is a 2-dimensional shape where every point curves inward like a saddle---opposite to the outward curvature of a sphere. Such surfaces come in infinitely many types, classified by their genus (number of holes) and the lengths of their boundaries. The moduli space of a given type is the collection of all possible hyperbolic surfaces of that type, treated as a single geometric object.

\textbf{Given.} You are given a surface of genus $g$ with $n$ boundary components of specified lengths $L_1, \ldots, L_n$.

\textbf{Goal.} Compute the volume of this landscape and discover whether it follows a simple polynomial pattern.

\textbf{Constraints.} The surface must be hyperbolic (constant negative curvature). The boundary lengths are prescribed. The genus and number of boundaries are fixed. The volume is measured using the Weil--Petersson metric, which is the natural geometric structure on the moduli space.

\textbf{Example.} For a once-punctured torus (genus 1, one boundary of length $L$), the moduli space volume is a simple polynomial in $L$. The question is whether this polynomial pattern persists for all genera and all boundary configurations, and whether a universal recursive formula exists to compute these volumes.
...
Mirzakhani proved that the volume $V_{g,n}(L_1, \ldots, L_n)$ of the moduli space of bordered Riemann surfaces with respect to the Weil--Petersson metric is a \emph{polynomial} in the boundary lengths $L_i$, and gave a recursion that computes it. The mechanism is the \textbf{earthquake flow}: a dynamical system on Teichmüller space that shears surfaces along geodesic laminations. Mirzakhani showed that the WP volume form can be written as an integral over the space of measured laminations. When a geodesic boundary is pinched (length $\to 0$), the surface degenerates into lower-genus components, and the integral decomposes accordingly. This yields a recursion: $V_{g,n}$ is expressed in terms of $V_{g',n'}$ for $(g',n') < (g,n)$. The polynomial nature of $V_{g,n}$ follows from the \textbf{intersection theory} on $\overline{\mathcal{M}}_{g,n}$: the recursion integrals evaluate to polynomial expressions in the $L_i^2$ via the Mumford relations.
...
\textbf{Why This Matters Beyond Surfaces.} The recursive formula has enabled explicit volume computations up to genus 50+, and the polynomial structure is now a testing ground for mirror symmetry predictions in string theory. Open questions include extending the recursion to higher Teichm\"uller theory for non-compact groups and understanding the deeper link between earthquake dynamics and quantum gravity path integrals.
