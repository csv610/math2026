...
\section{The Problem}
In 1742, Christian Goldbach wrote to Euler with a conjecture that has two parts:

\begin{enumerate}
\item \textbf{Strong Goldbach}: Every even integer $n \geq 4$ is the sum of two primes. (Still open!)
\item \textbf{Weak Goldbach}: Every odd integer $n \geq 7$ is the sum of three primes.
\end{enumerate}

The weak version is actually easier (by a remarkable irony) and was the one that was eventually solved.
