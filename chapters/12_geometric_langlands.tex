...
\section{The Problem}
The classical Langlands program (Chapter 11) relates Galois representations to automorphic forms over number fields and function fields. The \emph{geometric} Langlands program replaces:

\begin{itemize}
\item Number fields $\to$ Curves $X$ over a field $k$
\item Galois representations $\to$ Local systems (flat vector bundles) on $X$
\item Automorphic forms $\to$ D-modules (or perverse sheaves) on the stack of $G$-bundles on $X$
\item L-functions $\to$ Geometric correspondences
\end{itemize}

The central question: \emph{Is there a categorical equivalence between the derived category of D-modules on the stack of $G$-bundles and the derived category of coherent sheaves on the stack of local systems?}
