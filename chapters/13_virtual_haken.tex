...
\subsection{Informal}
Imagine you have a tangled, complicated three-dimensional shape---like a knotted ball of yarn frozen in space. A natural question arises: \emph{can you always find a surface living inside this shape that helps you understand it?} Specifically, is there always a two-dimensional sheet---think of it as a soap film stretched across a wire frame---that cuts through the shape in a way that reveals its internal structure?

Mathematicians call such a surface \emph{incompressible}: it is tightly fitted inside the shape, with no way to shrink it further without tearing. A 3-manifold that contains such a surface is called \emph{Haken}, after the German mathematician Wolfgang Haken who developed powerful algorithms for studying them. The question is whether \emph{every} complicated 3-manifold can be simplified in this way---perhaps after passing to a finite cover (a larger space that wraps around the original like a multi-story parking garage).

A 3-manifold $M$ is \emph{Haken} if it is compact, orientable, irreducible, and contains an embedded, two-sided, incompressible surface.
