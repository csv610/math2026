...
\subsection{Informal}
Some shapes live in higher-dimensional spaces where our everyday intuition breaks down. In 1960, Michel Kervaire discovered a special kind of ten-dimensional shape that could not be neatly tucked inside a sixteen-dimensional space, even though it seemed like it should fit. He devised a simple yes-or-no test for these shapes that turned out to be the key to a much deeper question: in which dimensions can such stubborn shapes exist? It took over fifty years, but mathematicians finally proved that beyond a certain point, these shapes can no longer appear.
