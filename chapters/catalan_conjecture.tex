...
Catalan's equation $x^a - y^b = 1$ asks: can two consecutive whole numbers both be perfect powers? The only example is $9 - 8 = 1$. The equation has four unknowns, but the real difficulty is the two \emph{exponents} $a$ and $b$. Think of the exponents as two dials on a combination lock. For 150 years, mathematicians could turn one dial at a time---fixing $a$ or $b$ and solving the resulting simpler equation---but nobody could turn both dials simultaneously.

Mihăilescu's insight was that the two dials are not independent. He discovered a hidden \emph{arithmetic relationship} between $a$ and $b$---the double Wieferich condition---that acts like a secret handshake: only certain pairs of primes can perform it, and among those, none can arise from a Catalan solution. The lock had only one combination all along, and it had already been found: $3^2 - 2^3 = 1$.
