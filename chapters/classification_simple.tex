...
\textbf{Background.} A symmetry of an object is a way to rearrange it so that it looks exactly the same afterward. Mathematicians collect all symmetries of an object into a structure called a group. A simple group is one that cannot be broken down into smaller groups---it is an ``atom'' of symmetry. The classification problem asks whether every simple group is one of a few known types.

\textbf{Given.} You are given any finite simple group---a symmetry structure that cannot be decomposed further.

\textbf{Goal.} Determine whether every finite simple group belongs to one of a known list: cyclic groups of prime order, alternating groups, groups of Lie type, or one of 26 exceptional sporadic groups.

\textbf{Constraints.} The group must be finite (contain finitely many elements) and simple (no nontrivial normal subgroups). The classification must be complete---no simple group may be missing from the list.

\textbf{Example.} The cyclic group of order 7 (the symmetries of a regular 7-gon, excluding reflections) is simple. The alternating group $A_5$ (the even permutations of 5 objects) is simple and is one of the smallest non-abelian simple groups. The Monster group, with approximately $8 \times 10^{53}$ elements, is the largest sporadic simple group. The classification says these---and only these---are all the simple groups that exist.
...
The \emph{groups of Lie type} are the finite symmetry groups that arise from algebraic groups over finite fields. They are the ``regular'' infinite families---analogous to how the prime numbers are the regular building blocks of arithmetic. Each family comes from a different type of geometric symmetry:
\begin{itemize}
\item $PSL_n(q)$: Groups of symmetries that preserve volume (``special linear'')---think of all the ways to rotate and stretch an $n$-dimensional space while keeping volume fixed, then reduce modulo a prime power $q$.
\item $PSU_n(q)$: Like $PSL_n(q)$ but preserving a Hermitian form---a symmetry that mixes complex conjugation with linear algebra.
\item $PSp_{2n}(q)$: Groups preserving a symplectic form---the kind of symmetry that appears in Hamiltonian mechanics (phase space).
\item $P\Omega_{2n+1}(q)$ and $P\Omega_{2n}^{\pm}(q)$: Groups preserving quadratic forms---the symmetries of ellipsoids and hyperboloids, reduced mod $q$.
\item Exceptional groups: $G_2(q)$, $^2F_4(q)$, $^3D_4(q)$, $E_6(q)$, $E_7(q)$, $E_8(q)$, $F_4(q)$---``exceptional'' because they do not fit into the infinite families above; they arise from exceptional Dynkin diagrams and have no analogue for arbitrary $n$.
\end{itemize}
\end{definition}

The groups of Lie type are parametrized by a prime power $q = p^f$ and a Dynkin diagram type (a small graph encoding the algebraic structure). They arise as the fixed points of Frobenius maps on algebraic groups---roughly, you take a continuous symmetry group over the complex numbers and ``reduce it modulo $q$'' to get a finite group.

\subsection{The Sporadic Groups}

The 26 sporadic groups are the ``exceptions''---finite simple groups that do not fit into any of the infinite families above. They are called \emph{sporadic} because they appear ``out of nowhere'': there is no parameter $n$ or $q$ that generates them, and no obvious pattern connecting them. Think of them as the mathematical equivalent of exceptional particles in physics---they exist, they are necessary for completeness, but they do not obey the rules that govern the regular families.
...
The classification works by induction on the \emph{involution structure} of the group: understanding how elements of order 2 behave lets you identify which family a group belongs to. If a group has an involution whose centralizer is ``thin'' (meaning it has very constrained 2-local structure), you can identify it as a known family---cyclic, alternating, or Lie type. The proof is organized as a tree: at each branch, you analyze the involution centralizer and either classify the group or reduce to a smaller case. By the 1980s, almost all branches were closed. The final gap was the \emph{quasi-thin} case: groups whose 2-local structure sits at the boundary between ``thin'' and ``not thin,'' where the centralizer has enough structure to resist existing classification methods but not enough to fall into known families. Aschbacher and Smith's 2,000-page argument systematically eliminated every remaining possibility in this borderline case by combining three threads: local analysis of 2-subgroups, permutation group methods, and representation-theoretic arguments. The result: every finite simple group belongs to one of the four known families, with no exceptions.
