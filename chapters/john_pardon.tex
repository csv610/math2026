...
\subsection{The Gap: Could Knot Distortion Be Unbounded?}

For over two decades after Gromov introduced knot distortion in the 1980s, a fundamental question remained open: \emph{can knots have arbitrarily large distortion, or is there an upper bound?} Mathematicians knew that some knots have large distortion---for example, certain iterated torus knots---but nobody could prove that distortion could be made arbitrarily large. The best known distortion bounds were polynomial in the crossing number, but nobody knew whether these bounds were tight or merely reflected the limitations of available techniques. It was conceivable that all knots had distortion bounded by some universal function of their combinatorial complexity.

\subsection{Why Existing Methods Failed}

The tools mathematicians had been using to study knot distortion were fundamentally \emph{combinatorial} and \emph{metric}: one would construct a specific knot, estimate its distortion by explicit calculation, and hope to push the estimate higher. These methods hit a wall for two reasons:

\begin{enumerate}
\item \textbf{The ``construction bottleneck'':} To prove distortion can be arbitrarily large, you need to construct a family of knots whose distortion grows without bound. But constructing explicit knots with high distortion requires simultaneously controlling both the intrinsic geometry (distances along the knot) and the extrinsic geometry (distances in $\mathbb{R}^3$). Every known construction faced a trade-off: making the knot more tangled in one region tended to simplify it elsewhere, keeping the overall distortion bounded.

\item \textbf{The ``measurement bottleneck'':} Even for knots that ``look'' highly tangled, proving that distortion is large is surprisingly hard. The distortion is an infimum over all embeddings, so one must show that \emph{no clever re-embedding} can simultaneously bring together all far-apart pairs of points. This is a global statement about all possible embeddings, which is much harder than analyzing a single embedding.
\end{enumerate}

The fundamental obstacle was that existing approaches treated distortion as a purely metric problem---a question about distances---and tried to solve it with metric tools. What was needed was a way to convert the metric question into a \emph{topological} or \emph{geometric} question, where the powerful machinery of modern geometry could be brought to bear.

\subsection{What Was Novel: The Asymptotic Conley--Zehnder Capacity}
...
Here is how to think about Pardon's argument at a high level:

\begin{quote}
\textbf{The analogy:} Imagine you have a tangled headphone cord. The \emph{distortion} asks: ``How much farther apart are two points along the cord than as the crow flies?'' A cord with high distortion has points that are neighbors along the wire but far apart in space.

\textbf{The old approach:} For decades, people tried to prove high distortion exists by constructing specific tangled knots and measuring their distortion directly---like trying to prove a number is large by writing it down. They could get distortion up to some polynomial bound, but couldn't push it to infinity.

\textbf{Pardon's insight:} Instead of measuring distortion directly, Pardon found a \emph{shadow} of distortion in a completely different branch of mathematics---symplectic geometry. The shadow is called the ``asymptotic Conley--Zehnder capacity,'' and it measures how much symplectic ``room'' a contact manifold needs. Pardon showed that if a knot has bounded distortion, its shadow must also be bounded. Then he constructed knots whose shadows grow without bound, forcing the distortion to grow without bound too.

\textbf{Why this worked:} The Conley--Zehnder capacity is a \emph{topological} invariant---it doesn't depend on the specific embedding, only on the abstract contact structure. This means Pardon could use the powerful tools of symplectic geometry (which are good at detecting topological obstructions) to answer a question about metric geometry (distances along embeddings). The key step was extending this invariant from convex bodies to all compact contact manifolds via the asymptotic construction.
\end{quote}
