% Chapter 13: John Pardon — Geometry, Topology, and Knots
...
\textbf{Background.} A knot is a loop of string tangled in space so that it cannot be undone without cutting. The distortion of a knot measures how much the knot ``stretches'' distances: it compares how close two points are along the string versus how far apart they are when measured through the surrounding space.

\textbf{Given.} You are given any knot---however complicated---tied in a loop of string and placed in three-dimensional space.

\textbf{Goal.} Determine whether there is an upper limit on how badly a knot can be distorted, or whether knots can be arbitrarily badly tangled in the sense of distortion.

\textbf{Constraints.} The knot must be a closed loop (no endpoints). The distortion is measured as the ratio of the longest distance along the string to the shortest distance through space, over all pairs of points on the knot. The bound must hold for all knots, with no exceptions.

\textbf{Example.} An untangled circle (the unknot) has distortion exactly 1: every pair of points is equally close along the string and through space. A simple trefoil knot has distortion slightly above 1. The question is whether, for any large number $C$ you name, you can tie a knot with distortion greater than $C$.
...
The knot distortion problem is fully settled: Pardon proved that distortion can be arbitrarily large, and the asymptotic Conley--Zehnder capacity is now a standard invariant in contact topology. No further cases or refinements of the original result are needed. The proof technique---translating a metric question into a topological one via symplectic invariants---was a one-shot innovation that resolved the question completely.

The tools Pardon developed have found applications well beyond knot theory. The asymptotic Conley--Zehnder capacity provides new obstructions to symplectic embedding problems (deciding when one symplectic manifold can fit inside another), which is a central question in symplectic geometry. His methods have also been applied to string theory, where strings moving through contact spaces are modeled as Legendrian submanifolds, and the capacity provides rigidity results for holomorphic curves. The connection between contact geometry and physics has become a thriving area of research.

Several related questions remain open. Finding optimal distortion bounds for specific knot types---how large the distortion must be for, say, a trefoil knot versus a figure-eight knot---is still largely unexplored. Higher-dimensional analogues of distortion, where one studies embeddings of higher-dimensional spheres, are only beginning to be understood. The interplay between Pardon's invariants and other measures of knot complexity, such as crossing number or bridge index, is another direction where much remains to be discovered.
