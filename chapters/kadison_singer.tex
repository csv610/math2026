...
Imagine you have a huge matrix and you want to split it into two halves so that neither half is too ``large'' (in a precise sense). You could try every possible split, but there are exponentially many, so that's hopeless. The brilliant idea of Marcus, Spielman, and Srivastava was to flip a coin for each row: heads means put it in the first half, tails in the second. Then they studied the \emph{average} behavior over all these random splits. They proved that the average is good enough that \emph{some specific split} must also be good---using a clever argument about how the roots of polynomials behave when you combine them. It's like showing that if the average temperature in 1,000 cities is comfortable, then at least one city must be comfortable.

\subsection{Marcus--Spielman--Srivastava (2013)}
