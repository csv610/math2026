% Chapter 30: Yu Deng — From Atoms to Fluids
...
\textbf{Background.} A gas is made of billions of tiny particles bouncing off each other. The Boltzmann equation describes how the gas evolves statistically---tracking the probability of finding a particle at a given position with a given velocity. The fluid equations (Euler or Navier--Stokes) describe the same gas as a smooth, continuous medium---like a river flowing without visible molecules.

\textbf{Given.} You are given a gas of hard spheres (billiard balls) governed by Newton's laws of motion. The gas starts in a state close to equilibrium (nearly uniform, with small perturbations).

\textbf{Goal.} Prove rigorously that, as the number of particles grows and the mean free path shrinks, the solutions of the Boltzmann equation converge to solutions of the fluid equations.

\textbf{Constraints.} The particles must interact via realistic hard-sphere collisions (not simplified models). The convergence must hold for a mathematically controlled time interval. The initial data must be close to equilibrium. The result must be a rigorous theorem, not a heuristic argument.

\textbf{Example.} Imagine a box filled with hard spheres that start nearly evenly distributed with nearly equal velocities. As time passes, the particles collide and the distribution evolves. The Boltzmann equation describes this evolution statistically. Zoom out far enough, and the gas should behave like a smooth fluid governed by the Euler equations. The question is whether this transition can be proved rigorously.
...
Imagine watching a flowing river from far away: the water moves smoothly as a single unit. Zoom in close and you see billions of individual molecules bouncing off each other like tiny billiard balls. Hilbert's sixth problem (1900) asked: can you start with the rules governing those individual collisions and \emph{prove} that the smooth river equations must follow? For over a century this was one of the hardest open problems in mathematical physics. Yu Deng's 2024 breakthrough with Hani and Ma finally proved it for hard-sphere gases. The core insight is ``propagation of chaos'': after enough collisions, different molecules become statistically independent---each one forgets its individual history and follows the crowd. Once you can prove this independence rigorously, with explicit error bounds, the smooth fluid equations emerge automatically from the microscopic chaos.

\subsection{What Was Stopping Progress}
The Boltzmann equation (1872) and the fluid equations (Euler, Navier--Stokes) were known individually, and physicists had long assumed that the former should follow from the latter in the right limit. But proving this rigorously---starting from Newton's laws for hard spheres and arriving at the Boltzmann equation with quantitative error bounds---had never been achieved for realistic molecular interactions. Previous progress was limited to simplified models: the Lorentz gas (particles bouncing off fixed obstacles rather than each other), linearized Boltzmann, or simplified kinetic models. The full chain from hard-sphere dynamics to Boltzmann resisted all attempts because controlling correlations between molecules over many collisions requires estimates far beyond what classical $L^2$ energy methods could provide.

\subsection{The Breakthrough}
Deng, Hani, and Ma introduced \emph{bilinear} energy estimates that exploit the product structure of the Boltzmann collision operator---instead of tracking each molecule independently (linear methods), they track pairs of molecules and how their correlations evolve. They combined this with the Dyson--Schwinger hierarchy, a technique borrowed from quantum field theory, to systematically expand the $N$-particle dynamics into a chain of equations for correlation functions. The key estimate shows that correlations decay exponentially in the number of collisions: after finitely many encounters, each molecule becomes statistically independent of the others. This quantitative propagation of chaos is the rigorous bridge from reversible Newtonian mechanics to irreversible fluid dynamics.
...
The derivation of the Boltzmann equation from hard-sphere dynamics is complete. Deng, Hani, and Ma proved in 2024--2025 that the empirical measure of an $N$-particle hard-sphere system converges to the Boltzmann equation with quantitative error bounds, resolving a special case of Hilbert's sixth problem. The wave kinetic equation derivation is also established.

These results have transformed the mathematical foundations of kinetic theory. The quantitative propagation of chaos---showing that correlations between molecules decay exponentially in the number of collisions---provides the first rigorous justification for the molecular independence assumption that underlies all of statistical mechanics. The bilinear energy estimates and Dyson--Schwinger hierarchy techniques introduced by Deng and collaborators have become new tools in the study of many-particle systems.

The full Hilbert sixth problem---deriving the Navier--Stokes equations from Newtonian mechanics---remains wide open. The current result covers hard-sphere gases with smooth initial data, but realistic molecular interactions involve softer potentials (attractive forces, Lennard-Jones interactions) that require fundamentally different analytic techniques. Connecting the Boltzmann equation to Navier--Stokes (the hydrodynamic limit) in the right regime is another major open challenge. These remaining problems sit at the frontier of mathematical physics and would complete the rigorous foundations of fluid mechanics.
