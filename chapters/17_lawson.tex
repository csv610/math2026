...
\chapter{The Lawson Conjecture}

\section{The Problem}
A soap film stretched across a wire frame seeks to minimize its surface area at every point---this is why soap bubbles are spherical and soap films on wire frames are flat where possible. A surface that achieves this local minimality is called \emph{minimal}: its mean curvature vanishes everywhere, meaning it curves equally in opposite directions at every point, like a saddle.

Now imagine a wire frame shaped like a donut---a torus---submerged in soapy water. The resulting soap film will be a minimal surface. The simplest such surface is the \emph{Clifford torus}, a perfectly symmetric donut that sits naturally inside a three-dimensional sphere. But are there other, more exotic minimal donuts? The Lawson conjecture, posed in 1970, says \emph{no}: the Clifford torus is the \emph{only} minimal donut that doesn't intersect itself.

A minimal surface in $S^3$ is a surface whose mean curvature vanishes identically. The Clifford torus
\[
T_{\text{Cl}} = \left\{ (z_1, z_2) \in \mathbb{C}^2 : |z_1| = |z_2| = \frac{1}{\sqrt{2}} \right\}
\]
is the simplest example of a minimal torus in $S^3$.
