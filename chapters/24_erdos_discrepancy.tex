...
\subsection{Informal}
In 1932, Paul Erdős asked a deceptively simple question:

\begin{quote}
Given an infinite sequence $(x_n)_{n=1}^\infty$ with $x_n \in \{-1, +1\}$, must the discrepancy
\[
D = \sup_{d, N \ge 1} \left| \sum_{k=1}^N x_{kd} \right|
\]
be infinite?
\end{quote}

In other words: no matter how cleverly you choose $\pm 1$ signs, can you keep the sums along \emph{all} arithmetic progressions bounded?

\begin{historicalbox}
Erdős offered \$500 for a solution. The problem resisted for 83 years. It is one of the simplest-to-state, hardest-to-solve problems in combinatorics. Erdős considered it one of his favorite problems.
\end{historicalbox}
