...
\section{The Problem}
In 1904, Henri Poincar\'e posed a question that would occupy mathematicians for over a century: \emph{Is every simply connected, closed, three-dimensional manifold homeomorphic to the three-sphere?}

Formally, let $M$ be a compact 3-manifold with no boundary. If the fundamental group $\pi_1(M)$ is trivial (i.e., every loop in $M$ can be continuously shrunk to a point), is $M$ necessarily homeomorphic to $S^3$, the standard 3-sphere?

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{diagrams/fundamental_group.pdf}
\caption{Visualizing the fundamental group: on $S^2$ every loop contracts to a point, but on the torus $T^2$ loops around the hole cannot be shrunk. The Poincar\'e Conjecture states $S^3$ is the only 3-manifold with this property.}
\label{fig:fundamental_group}
\end{figure}

In higher dimensions ($n \geq 5$), Stephen Smale proved the generalized Poincar\'e conjecture in 1961 using his h-cobordism technique \cite{smale1961}. The four-dimensional case was settled by Michael Freedman in 1982 \cite{freedman1982}. But the three-dimensional case remained stubbornly open.

\begin{historicalbox}
The three-dimensional case is the most famous because $S^3$ is the shape of our universe in the simplest cosmological model. Understanding whether $S^3$ is the \emph{only} simply connected 3-manifold is fundamental to topology.
\end{historicalbox}
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{diagrams/ricci_flow.pdf}
\caption{Ricci flow evolves a rough manifold toward a spherical shape. The equation $\partial g/\partial t = -2R$ drives the smoothing, and Perelman's entropy functionals control the process through singularities.}
\label{fig:ricci_flow}
\end{figure}
